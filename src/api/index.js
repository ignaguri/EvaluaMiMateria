...
export default {
  test() {
    return axios.get(URL + "api.php").then(response => {
      return response;
    });
  },
  checkLogin() {
  ...
  login(data) {
    return axios
      .get(
        URL + "usuarios" + "?filter=legajo,eq," + data.legajo + "&transform=1"
      )
      .then(response => {
  ...
  getEncuesta(id) {
    if (!this.checkLogin()) return Promise.reject(new Error("Not logged in"));
    return axios
      .get(URL + "encuestas/" + id + "?transform=1")
      .then(function(response) {
        return response.data;
      })
      .catch(function(error) {
  ...
  getEncuestaPorCodigo(codigo) {
    if (!this.checkLogin()) return Promise.reject(new Error("Not logged in"));
    return axios
      .get(URL + "encuestas" + "?filter=codigo,eq," + codigo + "&transform=1")
      .then(function(response) {
  ...
  getEncuestasPropietarias() {
    ...
    return axios
      .get(URL + "encuestas" + "?filter=creador,eq," + creador + "&transform=1")
      .then(creadas => {
  ...
  cambiarEtapa(idEncuesta, proxEtapa, fechaFin) {
    ...
    return axios
      .put(URL + "encuestas/" + idEncuesta, {
        etapaActual: idEtapa,
        fechaFinEtapaActual: new Date(fechaFin)
      })
      .then(r => {
  ...
  generarCodigo(idEncuesta, codigo) {
    if (!this.checkLogin()) return Promise.reject(new Error("Not logged in"));
    return axios
      .put(URL + "encuestas/" + idEncuesta, { codigo: codigo })
      .then(r => {
  ...
  matricularse(codigo) {
    if (!this.checkLogin()) return Promise.reject(new Error("Not logged in"));
    let alumno = this.checkLogin();
    return this.getEncuestaPorCodigo(codigo).then(found => {
      if (found) {
        return axios
          .post(URL + "usuariosxencuesta", {
            idEncuesta: found.idEncuestas,
            idUsuario: alumno
          })
          .then(r => {
            return true;
  ...
  getCriterio(id) {
    if (!this.checkLogin()) return Promise.reject(new Error("Not logged in"));
    return axios
      .get(URL + "criteriosxencuesta/" + id + "&transform=1")
      .then(r => {
        return r.data;
      })
      .catch(function(error) {
  ...
  getCriteriosXEncuestaConVotos(encuesta) {
    ...
    return axios
      ...
      .then(crit => {
        ...
        crit.forEach(c => {
          promesas.push(
            this.getVotosCriterio(c.idCriteriosXEncuesta, c.idEncuesta)
          );
        });
        return Promise.all(promesas);
      })
  ...
  getCriteriosDefinitivosConVotosXSemana(encuesta) {
    ...
    return axios
      ...
      .then(crit => {
        ...
        crit.forEach(c => {
          promesas.push(
            this.getRespuestasXSemana(c.idCriteriosXEncuesta, week)
          );
        });
        return Promise.all(promesas);
      })
  ...
  postCriteriosXEncuesta(criterio, encuesta) {
    ...
    return axios
      .post(URL + "criteriosxencuesta", body)
      .then(r => {
        return true;
      })
      .catch(error => {
  ...
  populateCriterioXEncuesta(e) {
    return this.getUsuario(e.propuestoPor).then(user => {
    ...
    return axios
      ...
      .then(r => {
        ...
        if (criterio.propuestoPor !== userId) {
          alert(
            "Error al borrar. El criterio no fue propuesto por el usuario intentando borrarlo."
          );
          return false;
        }
      ...
      .then(criterios => {
        const aBorrar = votosViejos.filter(voto => {
          for (let i = 0; i < criterios.length; i++) {
            if (criterios[i].idCriteriosXEncuesta === voto.idCriterioXEncuesta)
              return true;
          }
        });
        let promesas = [];
        aBorrar.forEach(c => {
          promesas.push(
            axios.delete(URL + "votosxcriterio/" + c.idVotosXCriterio)
          );
        });
        return Promise.all(promesas);
      })
  ...
  guardarCriteriosDefinitivos(criterios) {
    ...
    return axios
      .put(URL + "criteriosxencuesta/" + criterios.toString(), body)
      .then(r => {
  ...
  guardarEncuesta(criterio, voto) {
    ...
    return this.getRespuestasXSemana(criterio, semana)
      ...
      .then(r => {
        if (r) {
          return axios.put(
            URL + "respuestasxcriterio/" + r.idRespuestasXCriterio,
            body
          );
        } else {
